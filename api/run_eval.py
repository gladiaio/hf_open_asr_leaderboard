        ...
        try:
            ...
            if model_name.startswith(PREFIX):
                ...
                def make_request(url, headers, method="GET", data=None, files=None):
                    if method == "POST":
                        # For file uploads, use data parameter instead of json
                        if files:
                ...
                if use_url:
                    # For URL-based transcription - direct to pre-recorded endpoint
                    audio_url = sample["row"]["audio"][0]["src"]
                    ...
                    # For file-based transcription - first upload, then transcribe
                    with open(audio_file_path, "rb") as file_content:
                        # Set up headers for file upload (don't set Content-Type, let requests handle it)
                        upload_headers = {
                            "x-gladia-key": api_key,
                            "accept": "application/json",
                        }
